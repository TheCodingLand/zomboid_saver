...
class ZomboidBinaryParser:
    ...
    def read_value_by_type(self, value_type: int) -> Any:
        ...
        if value_type == 0x00:  # null or empty
            return None
        elif value_type == 0x01:  # double
            return self.read_double()
        elif value_type == 0x02:  # string
            return self.read_string()
        elif value_type == 0x03:  # table/map (skip for now)
            return None
        elif value_type == 0x04:  # boolean true
            return True
        elif value_type == 0x05:  # boolean false
            return False
        else:
            return None
...
def get_player_info(save_path: Path) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # First, try to get data from localPlayers table
        ...
        if row:
            ...
            # Parse binary data for additional info
            ...
            # Try to get hours and zombies from survivors table
            hours = 0
            zombies = 0
            try:
        ...
        # Fallback to survivors table (older format)
        cursor.execute("SELECT hours, zombiekills FROM survivors LIMIT 1")
        row = cursor.fetchone()
        if row:
            conn.close()
